...
# --- التحقق إذا سجل الطالب سابقًا ---
def check_student_already_registered(student):
    return str(student['رقم المذكرة']).strip() != ""

# --- التحقق إذا المذكرة مسجلة من قبل ---
def check_memo_already_registered(note_number, df_memos):
    memo = df_memos[df_memos["رقم المذكرة"].astype(str).str.strip() == str(note_number).strip()]
    if memo.empty:
        return False
    return str(memo.iloc[0]["تم التسجيل"]).strip() == "نعم"
...
def update_memo_registration(note_number, student1, student2=None):
    try:
        ...
        if row_idx.empty:
            ...
            return False
        idx = row_idx[0] + 2  # الصف في الشيت (Header =1)
        ...
        memo_updates.append({"range": f"Feuille 1!{chr(64+registration_col)}{idx}", "values": [["نعم"]]})
        memo_updates.append({"range": f"Feuille 1!{chr(64+date_col)}{idx}", "values": [[datetime.now().strftime('%Y-%m-%d %H:%M')]]})
        memo_updates.append({"range": f"Feuille 1!{chr(64+student1_col)}{idx}", "values": [[student1['اللقب'] + " " + student1['الإسم']]]})
        if student2 is not None:
            memo_updates.append({"range": f"Feuille 1!{chr(64+student2_col)}{idx}", "values": [[student2['اللقب'] + " " + student2['الإسم']]]})
        
        sheets_service.spreadsheets().values().batchUpdate(
            spreadsheetId=MEMOS_SHEET_ID,
            body={"valueInputOption": "USER_ENTERED", "data": memo_updates}
        ).execute()
        
        # تحديث شيت الطلبة
        student_updates = []
        student1_row = student1.name + 2
        student_updates.append({"range": f"Feuille 1!F{student1_row}", "values": [[note_number]]})
        if student2 is not None:
            student2_row = student2.name + 2
            student_updates.append({"range": f"Feuille 1!F{student2_row}", "values": [[note_number]]})
        
        sheets_service.spreadsheets().values().batchUpdate(
            spreadsheetId=STUDENTS_SHEET_ID,
            body={"valueInputOption": "USER_ENTERED", "data": student_updates}
        ).execute()
...
st.markdown("<h4 style='text-align:center;color:white;'>جامعة محمد البشير الإبراهيمي - برج بوعريريج<br>كلية الحقوق والعلوم السياسية</h4>", unsafe_allow_html=True)
...
if st.button("تسجيل الدخول"):
    ...
    if not valid1:
        ...
        if st.button("تأكيد تسجيل المذكرة"):
            # تحقق من التسجيل السابق للطلاب والمذكرة
            if check_student_already_registered(student1):
                st.error(f"❌ الطالب الأول ({student1['الإسم']}) سجل مذكرة من قبل!")
            elif memo_type == "ثنائية" and check_student_already_registered(student2):
                st.error(f"❌ الطالب الثاني ({student2['الإسم']}) سجل مذكرة من قبل!")
            elif check_memo_already_registered(note_number, df_memos):
                st.error("❌ هذه المذكرة مسجلة من قبل ولا يمكن تسجيلها مرة ثانية!")
            else:
                # المذكرة صالحة
                ...
                if not valid_memo:
                    ...
                    updated = update_memo_registration(note_number, student1, student2 if memo_type=="ثنائية" else None)
                    if updated:
