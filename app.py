...
import time
import textwrap # استيراد textwrap لضمان تنسيق الكود
...
.full-view-container {
    ...
    overflow: hidden; /* لضمان بقاء المحتوى داخله */
}
...
.student-card {
    flex: 1;
    max-width: 450px; /* منع التمدد الأفقي */
    min-width: 300px;
...
# ===========================================================
    ...
    if not st.session_state.logged_in:
        ...
        a_m = t_m - r_m; t_p = len(df_prof_memos["الأستاذ"].unique())
            # معالجة القيم الفارغة (Empty Strings) والفراغات
...
st.markdown("---")
st.markdown('<div style="text-align:center; color:#64748B; font-size:12px; padding:20px;">إشراف مسؤول الميدان : الدكتور رفاف لخضر</div>', unsafe_allow_html=True)
