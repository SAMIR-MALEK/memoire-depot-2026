        ...
        if st.session_state.get('selected_memo_id'):
            ...
            with tab5:
                ...
                if jury_memos.empty:
                    ...
                    # تصدير PDF في قسم منفصل
                    st.markdown("---")
                    with st.expander("📥 تصدير برنامج المناقشات PDF", expanded=False):
                        if st.button("⚙️ توليد PDF", key="gen_pdf_btn2", use_container_width=True):
                                ...
                                try:
                                    ...
                                    def _ar(text):
                                        ...
                                        try:
                                            ...
                                            r = get_display(arabic_reshaper.reshape(t))
                                            return r if r else t
                                        except: return t

                                    _fp = "/tmp/NotoSansArabic.ttf"
                                    _fb = "/tmp/NotoSansArabicBold.ttf"
                                    if not _os2.path.exists(_fp):
                                        _req2.urlretrieve("https://github.com/googlefonts/noto-fonts/raw/main/hinted/ttf/NotoSansArabic/NotoSansArabic-Regular.ttf", _fp)
                                    if not _os2.path.exists(_fb):
                                        _req2.urlretrieve("https://github.com/googlefonts/noto-fonts/raw/main/hinted/ttf/NotoSansArabic/NotoSansArabic-Bold.ttf", _fb)

                                    _pdf = FPDF(orientation="L", unit="mm", format="A4")
                                    _pdf.add_page()
                                    _pdf.set_auto_page_break(auto=True, margin=15)
                                    _pdf.set_margins(15, 15, 15)
                                    _pdf.add_font("Noto", "", _fp, uni=True)
                                    _pdf.add_font("Noto", "B", _fb, uni=True)

                                    _pdf.set_font("Noto", "B", 12)
                                    _pdf.set_text_color(15, 41, 66)
                                    _pdf.cell(0, 8, _ar("جامعة محمد البشير الإبراهيمي - برج بوعريريج"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
                                    _pdf.set_font("Noto", "", 10)
                                    _pdf.cell(0, 7, _ar("كلية الحقوق والعلوم السياسية"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
                                    _pdf.ln(3)
                                    _pdf.set_draw_color(15, 41, 66)
                                    _pdf.set_line_width(0.5)
                                    _pdf.line(15, _pdf.get_y(), 282, _pdf.get_y())
                                    _pdf.ln(4)
                                    _pdf.set_font("Noto", "B", 15)
                                    _pdf.set_text_color(15, 41, 66)
                                    _pdf.cell(0, 10, _ar("برنامج مناقشة مذكرات الماستر 2025-2026"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
                                    _pdf.ln(2)
                                    _pdf.set_font("Noto", "", 10)
                                    _pdf.set_text_color(80, 80, 80)
                                    _pdf.cell(0, 7, _ar(f"الأستاذ(ة): {prof_name}"), new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
                                    _pdf.ln(6)

                                    _cw = [15, 100, 30, 35, 22, 25]
                                    _hdrs = [_ar("رقم"), _ar("عنوان المذكرة"), _ar("الصفة"), _ar("تاريخ المناقشة"), _ar("التوقيت"), _ar("القاعة")]
                                    _pdf.set_font("Noto", "B", 9)
                                    _pdf.set_fill_color(15, 41, 66)
                                    _pdf.set_text_color(255, 255, 255)
                                    for _i, _h in enumerate(_hdrs):
                                        _pdf.cell(_cw[_i], 9, _h, border=1, align="C", fill=True)
                                    _pdf.ln()

                                    _pdf.set_font("Noto", "", 8)
                                    _pdf.set_text_color(30, 30, 30)
                                    _fill = False
                                    for _, _jm in filtered.iterrows():
                                        _id = str(_jm.get("رقم المذكرة","")).strip()
                                        _tt = str(_jm.get("عنوان المذكرة","")).strip()
                                        _rl = str(_jm.get("الصفة","")).strip()
                                        _dt = str(_jm.get("تاريخ المناقشة","")).strip()
                                        _tm = str(_jm.get("توقيت المناقشة","")).strip()
                                        _rm = str(_jm.get("القاعة","")).strip()
                                        _vals = [_id, _ar(_tt[:55]+("..." if len(_tt)>55 else "")), _ar(_rl),
                                                _ar(_dt if _dt not in ["","nan"] else "-"),
                                                _ar(_tm if _tm not in ["","nan"] else "-"),
                                                _ar(_rm if _rm not in ["","nan"] else "-")]
                                        _pdf.set_fill_color(248,250,252) if _fill else _pdf.set_fill_color(255,255,255)
                                        for _i,_v in enumerate(_vals):
                                            _pdf.cell(_cw[_i], 8, _v, border=1, align="C" if _i!=1 else "R", fill=True)
                                        _pdf.ln()
                                        _fill = not _fill

                                    _pdf.ln(5)
                                    _pdf.set_font("Noto", "", 8)
                                    _pdf.set_text_color(150,150,150)
                                    _pdf.cell(0, 6, _ar(f"تم التوليد: {datetime.now().strftime('%Y-%m-%d %H:%M')}"), align="C")
                                    st.session_state["_pdf_bytes"] = bytes(_pdf.output())
                                    st.success("✅ PDF جاهز!")
