...
def update_memo_registration(note_number, student1, student2=None):
    try:
        ...
        updates.append({"range": f"Feuille 1!{chr(64+student1_col)}{idx}", 
                        "values": [[student1['اللقب'] + " " + student1['الإسم']]]})
        # تسجيل الطالب الثاني إذا موجود
        if student2:
            student2_col = col_names.index("الطالب الثاني") + 1
            updates.append({"range": f"Feuille 1!{chr(64+student2_col)}{idx}", 
        ...
        updates.append({"range": f"Feuille 1!{chr(64+reg_col)}{idx}", "values": [["نعم"]]})
        updates.append({"range": f"Feuille 1!{chr(64+date_col)}{idx}", "values": [[datetime.now().strftime('%Y-%m-%d %H:%M')]]})
        ...
        st.error(f"❌ فشل تحديث حالة التسجيل في شيت المذكرات: {e}")
        return False
...
def update_student_memo(student, note_number):
    try:
        ...
        if row_idx.empty:
            ...
            return False
        idx = row_idx[0] + 2
        note_col = df.columns.tolist().index("رقم المذكرة") + 1
...
# ===== نافذة تسجيل المذكرة بعد الدخول =====
if st.session_state.get('logged_in', False):
    st.markdown("---")
    st.subheader("مرحلة تسجيل المذكرة")
    note_number = st.text_input("رقم المذكرة")
    memo_password = st.text_input("كلمة سر المذكرة", type="password")
    if st.button("تسجيل المذكرة"):
        student1 = st.session_state['student1']
        student2 = st.session_state.get('student2', None)
        ...
        if not valid_memo:
            ...
            updated1 = update_memo_registration(note_number, student1, student2)
            updated2 = update_student_memo(student1, note_number)
            updated3 = True
            if student2:
                updated3 = update_student_memo(student2, note_number)
            if updated1 and updated2 and updated3:
                st.success("✅ تم تسجيل المذكرة بنجاح! تم تحديث شيت المذكرات والطلبة.")
