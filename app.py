...
STUDENTS_RANGE = "Feuille 1!A1:K1000"  # شيت الطلاب
MEMOS_RANGE = "Feuille 1!A1:N1000"     # شيت المذكرات
...
def update_memo_registration(note_number, student1, student2=None):
    try:
        ...
        updates.append({"range": f"Feuille 1!{chr(64+registration_col)}{idx}", "values": [["نعم"]]})
...
# =======================
# تسجيل الدخول والتحقق من التسجيل السابق
# =======================
    ...
    if not valid1:
        ...
        if memo_type == "ثنائية":
            ...
            if not valid2:
                ...
                # تحقق من تسجيل مسبق
                if student1["رقم المذكرة"].strip() != "":
                    st.warning(f"⚠️ الطالب الأول {student1['الإسم']} سجل مذكرة من قبل: {student1['رقم المذكرة']}")
                elif student2["رقم المذكرة"].strip() != "":
                    st.warning(f"⚠️ الطالب الثاني {student2['الإسم']} سجل مذكرة من قبل: {student2['رقم المذكرة']}")
                else:
                    st.success(f"✅ تم تسجيل الدخول للطالبين: {student1['الإسم']} و {student2['الإسم']}")
        else:
            if student1["رقم المذكرة"].strip() != "":
                st.warning(f"⚠️ الطالب {student1['الإسم']} سجل مذكرة من قبل: {student1['رقم المذكرة']}")
            else:
        ...
        if st.button("تأكيد تسجيل المذكرة"):
            ...
            if not valid_memo:
                ...
                if updated:
                    st.success("✅ تم تسجيل المذكرة بنجاح! تم تحديث شيت المذكرات.")
